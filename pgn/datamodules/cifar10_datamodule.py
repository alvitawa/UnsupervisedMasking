...
class CIFAR10DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        root_dir = pathlib.Path(self.data_root) / 'cifar10'
        ...
        train_transform = torchvision.transforms.Compose([
            ...
            ) if self.resize_and_crop else torchvision.transforms.Resize(size=224),
        ...
        if stage == 'fit':
            self.train_set = CIFAR10(root=root_dir.as_posix(),
                                     download=True, train=True,
                                     transform=train_transform)

            self.val_set = CIFAR10(root=root_dir.as_posix(),
                                   download=True, train=False,
                                   transform=test_transform)

            self.train_set_unaugmented = CIFAR10(root=root_dir.as_posix(),
                                                 download=True, train=True,
                                                 transform=test_transform)

        if stage == 'test':
            self.test_set = CIFAR10(root=root_dir.as_posix(),
                                    download=True, train=False,
                                    transform=test_transform)
    ...
    def _load_id_to_class(self, root_dir):
        dummy_set = CIFAR10(root=root_dir.as_posix(),
                            download=True, train=True)
