...
class SVHNDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        root_dir = pathlib.Path(self.data_root) / 'svhn'
        ...
        if stage == 'fit':
            self.train_set = SVHN(root=root_dir.as_posix(),
                                  download=False, split='train',
                                  transform=train_transform)

            self.val_set = SVHN(root=root_dir.as_posix(),
                                download=False, split='test',
                                transform=test_transform)

            self.train_set_unaugmented = SVHN(root=root_dir.as_posix(),
                                                download=False, split='train',
                                                transform=test_transform)

        if stage == 'test':
            self.test_set = SVHN(root=root_dir.as_posix(),
                                 download=False, split='test',
                                 transform=test_transform)
    ...
    def _calculate_index_to_classes(self):
        self.index_to_classes = {
            idx: str(idx) for idx in range(self.nr_of_classes)
        }
