...
class Flowers102DataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.nr_of_classes = 102
        self.prompt_prefix = "This is a photo of a"

    def setup(self, stage: Optional[str] = None) -> None:
        root_dir = pathlib.Path(self.data_root) / 'flowers102'
        ...
        if stage == 'fit':
            ...
            self.train_set_unaugmented = JSONDataset(
                json_path=os.path.join(root_dir,
                                        'split_zhou_OxfordFlowers.json'),
                data_root=os.path.join(root_dir, 'images'),
                split='train',
    ...
    def _create_index_to_classes(self):
        index_to_classes = {
            key: value.replace("_", " ") for key, value in self.id_to_class.items()
        }
